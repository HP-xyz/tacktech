...
//************************************
Artemis_SQL_Handler::Artemis_SQL_Handler(std::map<std::string, std::string>& parameters)
{
	Artemis_SQL_Handler::parameters = parameters;
	Artemis_SQL_Handler::url =
		new std::string("tcp://" + parameters["database.address"]
						+ parameters["database.part"] );
	Artemis_SQL_Handler::user = new std::string(parameters["database.user"]);
	Artemis_SQL_Handler::password = new std::string(parameters["database.password"]);
	Artemis_SQL_Handler::database = new std::string(parameters["database.name"]);
...
//************************************
void Artemis_SQL_Handler::retrieve_and_print_resultset_metadata(sql::ResultSet* rs)
{
	if (rs -> rowsCount() == 0) 
	{
		throw std::runtime_error("ResultSetMetaData FAILURE - no records in the result set");
	}
	...
	std::cout << "\nNumber of columns in the result set = " << numcols << std::endl << std::endl;
	...
	for (int i = 0; i < numcols; ++i) 
	{
		...
		std::cout.width(20); 
		std::cout << res_meta -> getColumnDisplaySize (i+1) << std::endl << std::endl;
	}

	std::cout << "\nColumn \"" << res_meta -> getColumnLabel(1);
	std::cout << "\" belongs to the Table: \"" << res_meta -> getTableName(1);
	std::cout << "\" which belongs to the Schema: \"" << res_meta -> getSchemaName(1) << "\"" << std::endl << std::endl;
}
...
//************************************
void Artemis_SQL_Handler::retrieve_and_print_database_metadata( sql::Connection* dbcon)
{
	if (dbcon -> isClosed())
	{
	...
	std::cout << "Database Product Name: " << dbcon_meta -> getDatabaseProductName() << std::endl;
	std::cout << "Database Product Version: " << dbcon_meta -> getDatabaseProductVersion() << std::endl;
	std::cout << "Database User Name: " << dbcon_meta -> getUserName() <<std::endl << std::endl;

	std::cout << "Driver name: " << dbcon_meta -> getDriverName() << std::endl;
	std::cout << "Driver version: " << dbcon_meta -> getDriverVersion() << std::endl << std::endl;

	std::cout << "Database in Read-Only Mode?: " << dbcon_meta -> isReadOnly() << std::endl;
	std::cout << "Supports Transactions?: " << dbcon_meta -> supportsTransactions() << std::endl;
	std::cout << "Supports DML Transactions only?: " << dbcon_meta -> supportsDataManipulationTransactionsOnly() << std::endl;
	std::cout << "Supports Batch Updates?: " << dbcon_meta -> supportsBatchUpdates() << std::endl;
	std::cout << "Supports Outer Joins?: " << dbcon_meta -> supportsOuterJoins() << std::endl;
	std::cout << "Supports Multiple Transactions?: " << dbcon_meta -> supportsMultipleTransactions() << std::endl;
	std::cout << "Supports Named Parameters?: " << dbcon_meta -> supportsNamedParameters() << std::endl;
	std::cout << "Supports Statement Pooling?: " << dbcon_meta -> supportsStatementPooling() << std::endl;
	std::cout << "Supports Stored Procedures?: " << dbcon_meta -> supportsStoredProcedures() << std::endl;
	std::cout << "Supports Union?: " << dbcon_meta -> supportsUnion() << std::endl << std::endl;

	std::cout << "Maximum Connections: " << dbcon_meta -> getMaxConnections() << std::endl;
	std::cout << "Maximum Columns per Table: " << dbcon_meta -> getMaxColumnsInTable() << std::endl;
	std::cout << "Maximum Columns per Index: " << dbcon_meta -> getMaxColumnsInIndex() << std::endl;
	std::cout << "Maximum Row Size per Table: " << dbcon_meta -> getMaxRowSize() << " bytes" << std::endl;
	...
	while (rs -> next()) 
	{
		std::cout << "\t" << row << ". " << rs -> getString("TABLE_SCHEM") << std::endl;
		++row;
	} // while
...
std::string Artemis_SQL_Handler::create_resultset_xml()
{
	...
	size_t colom_count = Artemis_SQL_Handler::resultset->
	...
	while (Artemis_SQL_Handler::resultset->next()) 
	{
		...
		for (size_t i = 1; i <= colom_count; i++)
		{
			...
			node.append_child(pugi::node_pcdata)
                                .set_value(resultset->getString(i).c_str());
		}
	} // while
