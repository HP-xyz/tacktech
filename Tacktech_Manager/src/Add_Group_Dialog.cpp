...
{
	ui.setupUi(this);
	connect(ui.add_group_dialog_buttonbox, SIGNAL(accepted()), this,
			SLOT(ok_clicked()));
	connect(ui.add_group_dialog_buttonbox, SIGNAL(rejected()), this,
			SLOT(cancle_clicked()));
}
...
{
	if (group_and_computer_names->add_group_name(
			ui.group_name_line_edit->text().toStdString()))
	{
